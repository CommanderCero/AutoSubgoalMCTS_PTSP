...
public class ExperimentRandomness extends Exec
{
    ...
    private static void setup(String algorithm) throws Exception
    {
        ALGORITHM = algorithm;
        ...
        if(algorithm.equals("QD-MCTS"))
        {
            ...
            AutoSubgoalController.explorationRate = 14.93;
            AutoSubgoalController.maxRolloutDepth = 32;
            MCTSNoveltySearch.explorationRate = 15.43;
            MCTSNoveltySearch.maxSteps = 460;
        }
        else if(algorithm.equals("QD-MCTS-HP"))
        {
            // Handpicked parameter values that are probably not overfitted
            m_controllerName = "controllers.autoSubgoalMCTS.AutoSubgoalController";
            AutoSubgoalController.subgoalSearch = new MCTSNoveltySearch(4, new PositionBehaviourFunction(), AbstractController.rng);
            AutoSubgoalController.explorationRate = 4;
            AutoSubgoalController.maxRolloutDepth = 25;
            MCTSNoveltySearch.explorationRate = Math.sqrt(2);
            MCTSNoveltySearch.maxSteps = 460;
        }
        else if(algorithm.equals("S-MCTS"))
        {
            m_controllerName = "controllers.autoSubgoalMCTS.AutoSubgoalController";
            PositionGridPredicate predicate = new PositionGridPredicate(29, 3);
            RandomPredicateSearch.treatHorizonStatesAsSubgoals = true; // Now it behaves like S-MCTS
            AutoSubgoalController.subgoalSearch = new RandomPredicateSearch(predicate, 4, 513, AbstractController.rng);
            AutoSubgoalController.explorationRate = 13.81;
            AutoSubgoalController.maxRolloutDepth = 40;
        }
        else if(algorithm.equals("MS-MCTS"))
        {
            m_controllerName = "controllers.autoSubgoalMCTS.AutoSubgoalController";
            PositionGridPredicate predicate = new PositionGridPredicate(5, 3);
            RandomPredicateSearch.treatHorizonStatesAsSubgoals = false; // Now it behaves like MS-MCTS
            AutoSubgoalController.subgoalSearch = new RandomPredicateSearch(predicate, 4, 560, AbstractController.rng);
            AutoSubgoalController.explorationRate = 9.36;
            AutoSubgoalController.maxRolloutDepth = 22;
        }
        else if(algorithm.equals("VanillaMCTS"))
        {
            m_controllerName = "controllers.autoSubgoalMCTS.VanillaMCTS";
            VanillaMCTS.explorationRate = 7.80;
            VanillaMCTS.maxRolloutDepth = 35;
        }
        else if(algorithm.equals("VanillaGA"))
        {
            m_controllerName = "controllers.autoSubgoalMCTS.GeneticAlgorithm.GAController";
            GAController.GenomeLength = 35;
            GAController.PopulationSize = 151;
            GAController.MutationRate = 0.25;
        }
        else if(algorithm.equals("RB-MCTS"))
        {
            m_controllerName = "controllers.mcts.MCTSController";
            TSPGraphPhysicsEst.MAX_NODES = NUM_WAYPOINTS;
        }
        else
        {
