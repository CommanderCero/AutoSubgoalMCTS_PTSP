...
public class ScalarNSLCSearch implements ISubgoalSearch
{
    ...
    public int step(RewardGame state)
    {
        ...
        Genome<SearchData> bestGenome = evaluateGenomes(currPopulation);
        ...
        {
            // We have not improved in a while, add the best genome to the archive and restart the search
    ...
    {
        return steps >= maxSteps || subgoalArchive.size() > 5;
    }
    ...
    private Genome evaluateGenomesNoveltyAndReward(ArrayList<Genome<SearchData>> genomes)
    {
        ...
        for(int i = 0; i < genomes.size(); i++)
        {
            ...
            Collections.sort(genomes, (Genome<SearchData> v1, Genome<SearchData> v2) ->
            {
                double v1Dist = latentDist(v1.data.latentState, genome.data.latentState);
                double v2Dist = latentDist(v2.data.latentState, genome.data.latentState);
